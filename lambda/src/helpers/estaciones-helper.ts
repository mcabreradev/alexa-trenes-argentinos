...
export class EstacionesHelper {
  ...
  private static readonly ESTACIONES_COMUNES: Record<string, string> = {
    'retiro': 'RTR',
    'tigre': 'TIG',
    'constitucion': 'CST',
    'constitución': 'CST',
  ...
  static encontrarMejorCoincidencia(nombre: string, estaciones: Estacion[]): Estacion | null {
    ...
    const exacta = estaciones.find(e => 
      this.normalizarNombre(e.nombre) === normalizado
    );
    
    if (exacta) return exacta;
    
    // Buscar coincidencia parcial
    const parcial = estaciones.find(e => 
      this.normalizarNombre(e.nombre).includes(normalizado) || 
